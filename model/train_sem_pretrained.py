...
def main(opt):
    ...
    sem_seg_net = SegModel(opt)    
    #load pretrained model
    if opt.model != '':
        sem_seg_net = load_pretrain(sem_seg_net, opt.model)            
  

    # load the model for encoder    
    ae_net = MultiTaskNet(opt)
    if opt.ae_model != '':
        sem_seg_net.encoder = load_pretrain(ae_net.encoder, os.path.join(ROOT_DIR, opt.ae_model))
...
# start training
    ...
    for epoch in range(start_epoch, opt.n_epochs):
        ...
        for iter, data in enumerate(train_dataset):
            ...
            train_loss.backward()
            sem_seg_net.optimizer_encoder.step()
            sem_seg_net.optimizer_segmenter.step()
        ...
        if epoch%30==0 and epoch>0:
            sem_seg_net.update_learning_rate(0.5)
        ...
        with torch.no_grad():
            ...
            for iter, data in enumerate(val_dataset):
                ...
                #output
                sem_seg_net.encoder = sem_seg_net.encoder.eval()
                sem_seg_net.segmenter = sem_seg_net.segmenter.eval() 
                output=sem_seg_net(points_) #([8, 4096, n_classes])
